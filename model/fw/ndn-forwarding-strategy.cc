                                             ...
                                             bool didCreateCacheEntry)
{
  ...
  //StackHelper::AddRoute (node, "/data", inFace, 1); 
  
  //huyao 130510 不用data
  Name prefix = header->GetName ().cut (1); 
  m_fib->Add (prefix, inFace, 1);
  
  std::cout<<Simulator::GetContext ()<<":GetLastComponent () ： "<<prefix.GetLastComponent ()<<std::endl;
  ...
  //huyao 130510 不用data
  m_fib->Find (prefix)->UpdateCounter (inFace);

//暂时
 // m_fib->Begin ()->UpdateCounter (inFace);

  //fib::Entry::faceCounter[fib::Entry::index++] = inface->GetId();	
  //uint32_t i;
  //for(i=1; i<20; i++)
  //{
  //    fib::Entry::faceCounter[i] = fib::Entry::faceCounter[i-1];
  //}
  //fib::Entry::faceCounter[0] = inface->GetId();
}
  ...
  if (outgoing != pitEntry->GetOutgoing ().end ())
    {
      ...
      /*else if (outgoing->m_retxCount >= pitEntry->GetMaxRetxCount ())
        {
           std::cout<<Simulator::GetContext ()<<" Already forwarded before during this retransmission cycle (" <<outgoing->m_retxCount << " >= " << pitEntry->GetMaxRetxCount () << ")" <<std::endl;
        ...
        //huyao 130503 检测到retransmission时，flood到其他face
        else //if (name.find("c") < name.length()) //consumer 
        {
                Ptr<L3Protocol> ndn = this->GetObject<L3Protocol> ();
                std::cout<<Simulator::GetContext ()<<" : neighbours are "<<ndn->GetNFaces()<<" faces"<<std::endl;
                if (ndn->GetNFaces () > 2) //1个netface，一个appface
                {
                        //包括不在fib中的所有face
                        for (uint32_t faceNum = 0; faceNum < ndn->GetNFaces ()-1; faceNum++) //因为最后一个是appface，不是netface
                        {
                           Ptr<Face> outFace_ = ndn->GetFace (faceNum);

                           if (outFace != outFace_ && outgoing->m_retxCount < pitEntry->GetMaxRetxCount ())
                           {     
                                 //huyao 130510 不要data
                                 //Name name = Name ("/data");
                                 //m_fib->Find (name)->index = 0;   
                                 Name prefix = header->GetName ().cut (1);
                                 m_fib->Find (prefix)->index = 0;  //重头
                                                          
                                 if(TrySendOutInterest (inFace, outFace_, header, origPacket, pitEntry))
                                 {

                                        std::cout<<Simulator::GetContext ()<<": flood to face : "<<outFace->GetId()<<"  because of interest timed out "<<std::endl;
                                 }
                           }
                        }
                        return false;
