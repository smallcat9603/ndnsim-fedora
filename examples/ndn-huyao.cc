...
main (int argc, char *argv[])
{
  ...
  topologyReader.SetFileName ("src/ndnSIM/examples/topologies/topo-huyao.txt");
  topologyReader.Read ();
  ...
  for (int i = 0; i < 4; i++)
    {
      //huyao 130401 std::string prefix = "/data/"+Names::FindName (producers[i]);
      ...
      consumerHelper.SetAttribute ("LifeTime", StringValue ("2s")); // InterestLifetime
      
      consumerHelper.SetPrefix (prefix);
      ApplicationContainer consumer = consumerHelper.Install (consumers[i]);
      consumer.Start (Seconds (i*5));    // start consumers at 0s, 1s, 2s, 3s
      consumer.Stop  (Seconds (i*5+3)); // stop consumers at 19s, 18s, 17s, 16s
...
*/
  ...
  Ptr<Node> node1 = Names::Find<Node> ("n12");
  Ptr<Node> node2 = Names::Find<Node> ("n1");
  
  Ptr<NetDevice> device1 = node1->GetDevice(0);
  Ptr<NetDevice> device2 = node2->GetDevice(4);
